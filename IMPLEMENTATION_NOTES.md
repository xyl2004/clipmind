...
当前原型是一个 **Base-first 的 macOS Agent Wallet 上下文助手**。
...
当前版本只做上下文问答、信息查询和交易意图草稿，不会签名或广播真实交易。
...
| 输入类型 | Surf 命令 | 作用 |
| --- | --- | --- |
| 钱包地址 | `wallet-detail` | 查询 Base 钱包资产、代币、标签、授权 |
| 钱包地址 | `wallet-transfers` | 查询近期 Base 转账 |
| 代币合约 | `token-holders` | 查询 Base 代币主要持仓地址 |
| 代币合约 | `token-dex-trades` | 查询近期 Base DEX 交易 |
| 代币合约 | `token-transfers` | 查询近期 Base 代币转账 |
| 交易哈希 | `onchain-tx` | 查询 Base 交易详情 |
| 项目名称 | `project-detail` | 查询项目概览、代币信息、合约和社交信息 |
| 项目名称 | `search-news` | 查询相关新闻 |
| 全部查询 | `onchain-gas-price` | 查询 Base 当前 Gas |
...
视觉风格已经从经典 macOS `NavigationSplitView` / `List` 样式改为自定义产品界面：深色背景、半透明面板、强调色按钮、自绘会话历史和卡片式内容区。技术名保留英文，例如 Base、Surf、DEX、Gas、USDC，避免加密产品里常见名词被翻译后变得不自然。
...
当选中的文本是地址、交易哈希或短项目名时，应用会尝试补充 Surf/Base 数据后再让 AI 回答。普通长文本则直接作为上下文发给 LLM。
...
1. 接入 Base swap 报价，但仍然只生成确认单。
2. 增加策略检查：最大金额、滑点上限、黑名单、合约风险提示。
3. 最后再接本地签名器或智能账户，不让 LLM 接触私钥。
